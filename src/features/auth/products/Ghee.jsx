...
export default function Ghee() {
  const [gheeList, setGheeList] = useState([]);
  ...
  useEffect(() => {
    const fetchGhees = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "products"));

        const ghees = querySnapshot.docs
          .map((doc) => ({
            id: doc.id,
            ...doc.data(),
          }))
          .filter((item) => item.type?.toLowerCase() === "ghee");

        setGheeList(ghees);
      } catch (error) {
        console.error("Error fetching ghee:", error);
      }
    };
    fetchGhees();
  }, []);
    ...
    <>
      <div className="min-h-screen bg-gray-100 pt-10 pb-20 px-4 md:px-8">
        <div className="px-10 mx-auto">
          <h1 className="text-4xl font-bold text-green-600 mb-10">Pure Ghee</h1>

          {gheeList.length === 0 ? (
            <div className="text-center text-gray-500 text-xl">
              No Ghee Products Available
            </div>
          ) : (
